...
type Props = { todo?: Todo; deleted?: boolean; onSave: (input: TodoUpdate) => Promise<void>; onClose: () => void }
...
export default function TodoEditor({ todo, deleted = false, onSave, onClose }: Props) {
  ...
  useEffect(() => {
    if (deleted) dialog.current?.scrollTo(0, 0)
  }, [deleted])
  ...
  async function submit(event: FormEvent) {
    ...
    setTaskTouched(true)
    if (deleted || savingRef.current || Object.keys(errors).length || deadlineError) return
  ...
  return <dialog ref={dialog} className="editor-modal" aria-labelledby="editor-title"
    ...
    <form onSubmit={submit} noValidate>
      ...
      {deleted && <div className="save-error deleted-notice" role="alert"><AlertCircle size={18} /><span>This Todo was deleted. Unsaved input has not been saved.</span></div>}
      <fieldset className="editor-fields" disabled={saving && !deleted}>
        <div className="field">
          ...
          <textarea id="todo-task" value={draft.task} readOnly={deleted} onChange={event => updateDraft({ task: event.target.value })} onBlur={() => setTaskTouched(true)} rows={2} autoFocus aria-invalid={Boolean(taskError)} aria-describedby={taskError ? 'task-error' : undefined} />
        ...
        <div className="field">
          ...
          <textarea id="todo-description" value={draft.description} readOnly={deleted} onChange={event => updateDraft({ description: event.target.value })} rows={4} aria-invalid={Boolean(errors.description)} aria-describedby={errors.description ? 'description-error' : undefined} />
        ...
        <div className="field">
          ...
          <div className="deadline-input">
            <input id="todo-due-at" type="datetime-local" value={deadline} disabled={deleted} onChange={event => changeDeadline(event.target.value, event.target.validity.badInput)} aria-invalid={deadlineError} aria-describedby={deadlineError ? 'deadline-error' : undefined} />
            <button className="icon-button" type="button" title="Clear deadline" aria-label="Clear deadline" onClick={() => changeDeadline('')} disabled={deleted || !deadline}><X size={17} /></button>
          </div>
          {deadlineError && <p id="deadline-error" className="field-error">Enter a valid local date and time.</p>}
        </div>
        <fieldset className="status-field" disabled={deleted}><legend>Status</legend>{overdue
      ...
      {failure && !deleted && <div className="save-error" role="alert"><AlertCircle size={18} /><span>{failure}</span></div>}
      <footer className="modal-footer"><button className="button secondary" type="button" disabled={saving} onClick={onClose}>Cancel</button>
        <button className="button primary" type="submit" disabled={deleted || saving || Object.keys(errors).length > 0 || deadlineError}>{saving ? <LoaderCircle className="spin" size={17} /> : <Save size={17} />}{saving ? 'Saving' : 'Save todo'}</button>
      </footer>
    </form>
  </dialog>
}